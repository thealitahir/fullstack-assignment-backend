...
function sendError(err, res) {
  ...
  if (typeof err == "object") {
    ...
    if (err.name == "SequelizeHostNotFoundError") {
      res
        ...
        .send({
          status: code,
          success: false,
          errors: [{ message: "DB Connection Issue." }]
        });
      ...
      res
        .status(code)
        .send({ status: code, success: false, errors: err.errors });
      ...
      res
        .status(code)
        .send({ status: code, success: false, errors: response.errors });
    } else {
      res
        .status(code)
        .send({
    ...
    res
      .status(422)
      .send({ status: 422, success: false, errors: [{ message: err }] });
  }
}
